...
namespace hy_manipulation_controllers
{
  ...
  KinematicsSolver::KinematicsSolver(const std::string &_urdf_path)
  {
    ...
    if (doc.LoadFile(_urdf_path.c_str()) != tinyxml2::XML_SUCCESS)
    {
      throw std::runtime_error("Failed to load URDF file with XML parser: " + _urdf_path);
    }
    tinyxml2::XMLElement *robot_node = doc.FirstChildElement("robot");
    if (!robot_node)
    {
      throw std::runtime_error("URDF does not contain a <robot> tag.");
    }
    joint_params_.clear();
    joint_params_.reserve(joint_names.size());
    for (const auto &name : joint_names)
    {
      JointControlParams params;
      params.id = joint_params_.size();

      bool found = false;
      for (tinyxml2::XMLElement *joint_node = robot_node->FirstChildElement("joint"); joint_node; joint_node = joint_node->NextSiblingElement("joint"))
      {
        const char *joint_name_attr = joint_node->Attribute("name");
        if (joint_name_attr && name == joint_name_attr)
        {
          tinyxml2::XMLElement *limit_node = joint_node->FirstChildElement("limit");
          if (limit_node)
          {
            params.max_vel = limit_node->FloatAttribute("velocity", 1.0f);
            const char *acc_attr = limit_node->Attribute("acceleration");
            if (acc_attr)
            {
              params.max_acc = limit_node->FloatAttribute("acceleration");
            }
            else
            {
              params.max_acc = 2.5f * params.max_vel;
              LOG_INFO("Acceleration limit for joint '%s' not found in URDF. Heuristically set to %.2f (2.5 * max_velocity).",
                       name.c_str(), params.max_acc);
            }
            found = true;
            break;
          }
      ...
      if (!found)
      {
        LOG_WARN("Could not find <limit> tag for joint '%s' in URDF. Using default values (vel=%.1f, acc=%.1f).",
                 name.c_str(), params.max_vel, params.max_acc);
      }
  ...
  用直线近似加速度和位置之间的关系
  */
  void KinematicsSolver::AdjustEndpointAccelerations(const int n, const double dt[], double x[], double x1[], double x2[])
  {
    float a_init = 0.0f; // 可以根据实际的改
    float a_fina = 0.0f;
    // 存储原始位置作为参考
  ...
  void KinematicsSolver::FitCubicSpline(const int n, const double dt[], const double x[], double x1[], double x2[])
  {
      ...
      const float acc_duration)
  {
    ...
    {
      LOG_ERROR("Stored joint parameters in KinematicsSolver do not match joint count.");
      return false;
    }
    ...
    for (size_t i = 0; i < num_joints; ++i)
    {
      ...
      double desired_acceleration = (acc_duration > 1e-4) ? (target_velocity / acc_duration) : joint_params_[i].max_acc; // 防止除以0
      max_accelerations[i] = std::min(static_cast<double>(joint_params_[i].max_acc), desired_acceleration);
    }
    ...
    // 初始化时间参数 并猜测预估时间
    out_time_diff.assign(num_points - 1, std::numeric_limits<double>::epsilon()); // 用一个很小的正数初始化
    for (size_t j = 0; j < num_joints; ++j)
    {
      for (size_t i = 0; i < num_points - 1; ++i)
      {
        double dx = t2[j].positions[i + 1] - t2[j].positions[i];
        double time_needed = (std::abs(dx) > 1e-6) ? std::abs(dx / (dx >= 0 ? t2[j].max_velocity : t2[j].min_velocity)) : 0.0;
    ...
    while (1)
    {
      ...
      for (size_t j = 0; j < num_joints; ++j)
      {
        if (num_points >= 4)
        {
          for (size_t j = 0; j < num_joints; ++j)
          {
            AdjustEndpointAccelerations(num_points, out_time_diff.data(), t2[j].positions.data(), t2[j].velocities.data(), t2[j].accelerations.data());
          }
        }
        FitCubicSpline(num_points, out_time_diff.data(), t2[j].positions.data(), t2[j].velocities.data(), t2[j].accelerations.data());
        for (size_t i = 0; i < num_points; ++i)
        { // 调整系数 防止加速度超幅
    ...
    for (size_t j = 0; j < num_joints; ++j)
    {
      ...
      for (size_t i = 0; i < num_points; ++i)
      {
        if (i == 0)
        { // 确保 Eigen 向量已初始化大小
          sparse_waypoints[i].velocity.resize(num_joints);
          sparse_waypoints[i].acceleration.resize(num_joints);
        }
      ...
      float control_frequency)
  {
    ...
    if (waypoints_to_process.size() >= 2)
    {
      ...
      start_helper.position = (9.0 * waypoints_to_process[0].position + 1.0 * waypoints_to_process[1].position) / 10.0;
      waypoints_to_process.insert(waypoints_to_process.begin() + 1, start_helper);

      SparseWaypoint end_helper;
      end_helper.position = (1.0 * waypoints_to_process[waypoints_to_process.size() - 2].position + 9.0 * waypoints_to_process.back().position) / 10.0;
      waypoints_to_process.insert(waypoints_to_process.end() - 1, end_helper);
    }

    std::vector<double> time_diff;

    if (!ComputeTimeOptimalProfiles(waypoints_to_process, time_diff, vel_percentage, acc_duration))
    {
    ...
    double total_duration = std::accumulate(time_diff.begin(), time_diff.end(), 0.0);
    ...
    trajectory.reserve(static_cast<size_t>(total_duration * control_frequency) + num_points);
    ...
    for (size_t i = 0; i < num_points - 1; ++i)
    {
      ...
      for (size_t j = 0; j < num_joints; ++j)
      {
        splines[j].SetProfileDuration(
            waypoints_to_process[i].position(j), waypoints_to_process[i].velocity(j), waypoints_to_process[i].acceleration(j),
            waypoints_to_process[i + 1].position(j), waypoints_to_process[i + 1].velocity(j), waypoints_to_process[i + 1].acceleration(j),
            seg_duration);
      }

      const int num_steps_in_segment = static_cast<int>(std::round(seg_duration / dt));
    ...
    LOG_INFO("Successfully interpolated trajectory with %zu points over %.2f seconds.",
