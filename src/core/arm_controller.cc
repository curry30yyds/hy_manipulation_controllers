...
namespace hy_manipulation_controllers
{
  ...
  bool ArmController::GetLinkInfo(std::vector<KinematicsSolver::JointLinkInfo> &infos)
  {
    if (!kinematics_solver_->ExtractLinkJointInfo(urdf_path, infos))
    {
      LOG_ERROR("Can not load the links message !");
      return false;
    }
    return true;
  }
